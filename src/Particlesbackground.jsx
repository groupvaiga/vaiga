...
class Particle {
  ...
  update() {
    ...
    if (this.twinkle) {
      ...
      this.size = Math.max(0.1, this.baseSize + Math.sin(this.twinklePhase) * 0.3)
    }
  ...
  draw(ctx) {
    ...
    for (let i = 0; i < this.trail.length; i++) {
      ...
      ctx.beginPath()
      ctx.arc(t.x, t.y, Math.max(0.1, this.size * ratio * 0.65), 0, Math.PI * 2)
      ctx.fillStyle = `rgba(255,255,255,${this.alpha * ratio * 0.4})`
      ctx.fill()
    }

    // core dot with white glow
...
// ── Main wrapper export ───────────────────────────────────────
export default function ParticlesBackground({ children, style }) {
  return (
    <div
      style={{
        position: 'relative',
    ...
    >
      ...
      {/* children render above the canvas */}
...
/*
  ...
  │                                                     │
  │  TWEAKS (top of file):                              │
  │  • NUM          → particle count (default 200)     │
  │  • baseSize     → dot size range                   │
  │  • speedY       → rising speed                     │
  │  • trailLen     → comet tail length                │
  │  • alpha range  → overall brightness               │
  │  • '#07070f'    → background colour                │
  └─────────────────────────────────────────────────────┘
*/